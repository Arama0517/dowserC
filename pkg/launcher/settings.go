...
func InitSettings() error {
	...
	// 定义启动器配置文件路径
	SettingsPath = filepath.Join(CWD, "launcher-settings.json")
	DowserPath = filepath.Join(CWD, "dowser.exe")

	// 检查dowser是否存在
	switch _, err := os.Stat(DowserPath); {
	...
	if settings.GameDataPath != DataDir {
		...
		if err := os.WriteFile(SettingsPath, byteData, 0o755); err != nil {
